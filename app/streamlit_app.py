...
def render_login():
    ...
    with st.expander("📖 이 시뮬레이터는 무엇인가요? (데이터 출처 · 시뮬레이션 방법 안내)"):
        st.markdown(
            """
**무엇을 하는 도구인가요?**

학령인구 감소로 소규모화된 학교의 통폐합 여부를 검토할 때, "학생수 추이"와
"통학 이동시간" 두 가지 정량 데이터를 결합한 공간최적화 모형(CPMP)으로
통폐합 후보 조합과 그 영향(통학시간 변화 등)을 데이터에 근거해 확인할 수 있게
해주는 시뮬레이터입니다. 1차 MVP로, 초등학교·중학교(고등학교 제외)를 대상으로
경기도 전역에서 시·군 단위로 조회할 수 있습니다.

**데이터는 어디서 구했나요?**

| 항목 | 출처 |
|---|---|
| 학생수·학급수·교사수(2014~2026) | 2014~2024년 초중등 교육통계자료 + 2025/2026년 학년별·학급별 학생수 스냅샷 |
| 학교 위치(위경도) | 공공데이터포털 "전국초중등학교위치표준데이터" |
| 학생 거주정보(수요지) | 국토교통부 국토정보플랫폼 500m×500m 격자 인구(개인정보 비식별 집계값) |
| 통학 이동시간 | 카카오모빌리티 길찾기 API 실제 도로망 이동시간(선택 지역에서 온디맨드 계산·캐시). 아직 계산 전인 지역은 직선거리 기반 근사치를 임시로 사용하며, 화면에 실측/근사 여부를 항상 표시합니다 |
| 시·군구 행정경계 | 통계청 SGIS(Statistics Geographic Information Service) Open API |
| 학생1인당 교육활동비 | 학교회계/사립학교 교비회계 예·결산서 세출자료(2025년 기준) |

**시뮬레이션은 어떻게 작동하나요?**

CPMP(Capacitated p-median Problem, 수용력 제한 p-중위 문제)라는 공간최적화
모형을 사용합니다. 쉽게 말해, 남길 학교 수(p)를 정하면, 각 학생 거주지(격자)를
"어느 학교에 배정해야 전체 통학시간(이동시간 × 학생수의 합)이 최소가 되는가"를
계산해주는 방식입니다.

1. **5단계**에서 세 가지 파라미터를 정합니다 — 잔존 학교 수(p), 학교별 수용
   인원 상한, 허용 가능한 최대 통학시간(분)
2. 모형이 모든 학생 거주지를 남을 학교들 중 하나에 배정하되, 각 학교의 수용
   상한을 넘지 않고, 총 가중 통학시간이 최소가 되는 조합을 찾습니다
3. 만약 이동시간 제한 안에 도달 가능한 학교가 하나도 없는 거주지가 있으면,
   억지로 배정하지 않고 "실행 가능한 해 없음"으로 알려드립니다 — 통학 사각지대를
   숨기지 않기 위한 정책입니다
4. **6단계**에서 통합 전(현재 상태)과 통합 후를 나란히 비교해 보여줍니다

세 파라미터(잔존 학교 수·수용상한·이동시간 제한)는 지역마다 적정값이 다를 수
있어 5단계에서 직접 조정하며 여러 시나리오를 비교해 볼 수 있습니다.
            """
        )
...
def render_step6():
    ...
    if result is None or result.status != "solved":
        ...
        with f1:
            if st.button("← 5단계로", key="b6back"):
                goto(5); st.rerun()
        with f2:
            if st.button("🏠 메인으로 돌아가기(지역 다시 선택)", key="b6home-empty"):
                goto(1); st.rerun()
        return
    ...
    with e1:
        if st.button("← 5단계로 돌아가 다른 시나리오 비교", key="b6back2"):
            goto(5); st.rerun()
    with e2:
        if st.button("🏠 메인으로 돌아가기(지역 다시 선택)", key="b6home"):
            goto(1); st.rerun()
